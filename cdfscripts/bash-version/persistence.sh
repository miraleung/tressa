...
SRC=`pwd`/asserts
PATH=`pwd`
TMPFILE=$PATH/persist-temp.txt
NCOMMITS_DSTFILE=$PATH/persistence-num-commits.txt
DAYS_DSTFILE=$PATH/persistence-days.txt
DATA_DSTFILE=$PATH/persistence-data.txt
...
# Build map of asserts (predicates)
declare -A ASSERTMAP
